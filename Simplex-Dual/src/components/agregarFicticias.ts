import type { FuncionObjetivo, Inecuacion } from "@/interfaces/interfaceSimplex";

type Ecuacion = number[];

export const convertirInecuacionesAEcuaciones = (inecuaciones: Inecuacion[]): Ecuacion[] => {
    ...
    const nuevasEcuaciones = inecuaciones.map((inecua) => {
        let ecuacionPlano: Ecuacion = [inecua.x1, inecua.x2];
...
export const convertirFO = (inecuaciones: Inecuacion[], funcionObjetivo:FuncionObjetivo): Ecuacion => {
    let funcionObjetivoWithFicticias = [funcionObjetivo.x1, funcionObjetivo.x2];
    inecuaciones.forEach((inecua) => {
        if (inecua.restriccion === 0) {
